...
import com.example.playlistmaker.ui.tracks.models.ResultOfAddingState.ALREADY_EXISTS
...
class AudioPlayerActivity (
    ...
    private suspend fun setupBottomSheet() {
        ...
        val playlists = viewModel.getListOfPlaylist().toMutableList()
        binding.recycleViewBottomSheet.layoutManager = LinearLayoutManager(this)

        adapter = PlaylistsAdapterBottomSheet(this, playlists,object : PlaylistsAdapterBottomSheet.Listener {
    ...
    private fun setupViewModelObservers() {
        viewModel.apply {
            ...
            getIsFavourite().observe(this@AudioPlayerActivity, ::changeFavouriteIcon)



            lifecycleScope.launch {
                changeFavouriteIcon(checkIfTrackIsFavorite(track.trackId))
            }
    ...
    suspend fun methodToCallFromFragment() {
        ...
        val playlists = viewModel.getListOfPlaylist()
        adapter.updateData(playlists)
